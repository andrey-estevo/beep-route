...
export function AddressEditor({ pack, onSave, onDelete, onClose }: Props) {
  ...
  function save() {
    ...
    onSave({ ...pack, recipient: recipient.trim() || undefined, vehiclePosition: vehiclePosition.trim() || undefined, address: selected.label, latitude: selected.latitude, longitude: selected.longitude, status: "ready" });
  }
  ...
  return <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Endereço da encomenda"><section className="address-modal">
    <header><div><small>PACOTE {pack.trackingCode}</small><h2>Confirmar endereço</h2></div><button type="button" onClick={onClose} aria-label="Fechar">×</button></header>
