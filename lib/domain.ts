...
export function addPackage(route: RouteState, trackingCode: string): { route: RouteState; duplicate: boolean } { const code = normalizeTrackingCode(trackingCode); if (route.packages.some((item) => item.trackingCode === code)) return { route, duplicate: true }; const pack: DeliveryPackage = { id: crypto.randomUUID(), trackingCode: code, address: "", latitude: 0, longitude: 0, status: "address_pending" }; return { route: { ...route, packages: [pack, ...route.packages] }, duplicate: false }; }
export function addDemoPackage(route: RouteState, trackingCode: string) { const result = addPackage(route, trackingCode); if (result.duplicate) return result; const location = demoAddresses[route.packages.length % demoAddresses.length]; const [pack, ...rest] = result.route.packages; return { duplicate: false, route: { ...result.route, packages: [{ ...pack, address: location.address, latitude: location.latitude, longitude: location.longitude, status: "ready" as const, vehiclePosition: route.packages.length % 2 ? "Caixa A" : "Porta-malas esquerdo" }, ...rest] } }; }
export function groupStops(packages: DeliveryPackage[]): DeliveryStop[] { const groups = new Map<string, DeliveryPackage[]>(); for (const item of packages.filter((pack) => pack.status !== "delivered" && pack.status !== "address_pending" && pack.latitude !== 0 && pack.longitude !== 0)) { const key = `${item.latitude.toFixed(5)}:${item.longitude.toFixed(5)}`; groups.set(key, [...(groups.get(key) ?? []), item]); } return [...groups.values()].map((items, index) => ({ id: crypto.randomUUID(), sequence: index + 1, address: items[0].address, latitude: items[0].latitude, longitude: items[0].longitude, status: "pending", packages: items })); }
