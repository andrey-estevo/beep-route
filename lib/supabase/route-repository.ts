...
interface PackageRow { id: string; stop_id: string | null; tracking_code: string; barcode_raw: string | null; recipient_name: string | null; address_full: string | null; latitude: number | null; longitude: number | null; status: PackageStatus; vehicle_position: string | null }
...
export async function loadRemoteRoute(userId: string): Promise<RouteState | null> {
  ...
  const [{ data: packageData, error: packageError }, { data: stopData, error: stopError }] = await Promise.all([
    supabase.from("packages").select("id,stop_id,tracking_code,barcode_raw,recipient_name,address_full,latitude,longitude,status,vehicle_position").eq("route_id", row.id).order("scanned_at", { ascending: false }),
  ...
  const packages: DeliveryPackage[] = packageRows.map((item) => ({ id: item.id, trackingCode: normalizeTrackingCode(item.tracking_code), rawBarcode: item.barcode_raw ?? undefined, recipient: item.recipient_name ?? undefined, address: item.address_full ?? "", latitude: item.latitude ?? 0, longitude: item.longitude ?? 0, status: item.status, vehiclePosition: item.vehicle_position ?? undefined }));
