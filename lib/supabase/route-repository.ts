...
interface PackageRow { id: string; stop_id: string | null; tracking_code: string; barcode_raw: string | null; marketplace_stop_number: number | null; recipient_name: string | null; address_full: string | null; latitude: number | null; longitude: number | null; status: PackageStatus; vehicle_position: string | null }
interface StopRow { id: string; sequence: number; marketplace_stop_number: number | null; address_full: string; latitude: number; longitude: number; status: StopStatus }
...
export async function loadRemoteRoute(userId: string): Promise<RouteState | null> {
  ...
  const [{ data: packageData, error: packageError }, { data: stopData, error: stopError }] = await Promise.all([
    supabase.from("packages").select("id,stop_id,tracking_code,barcode_raw,marketplace_stop_number,recipient_name,address_full,latitude,longitude,status,vehicle_position").eq("route_id", row.id).order("scanned_at", { ascending: false }),
    supabase.from("route_stops").select("id,sequence,marketplace_stop_number,address_full,latitude,longitude,status").eq("route_id", row.id).order("sequence"),
  ]);
  if (packageError) throw packageError;
  if (stopError) throw stopError;
  const packageRows = (packageData ?? []) as PackageRow[];
  const packages: DeliveryPackage[] = packageRows.map((item) => ({ id: item.id, trackingCode: normalizeTrackingCode(item.tracking_code), rawBarcode: item.barcode_raw ?? undefined, marketplaceStopNumber: item.marketplace_stop_number ?? undefined, recipient: item.recipient_name ?? undefined, address: item.address_full ?? "", latitude: item.latitude ?? 0, longitude: item.longitude ?? 0, status: item.status, vehiclePosition: item.vehicle_position ?? undefined }));
  ...
  const stops: DeliveryStop[] = ((stopData ?? []) as StopRow[]).map((item) => ({ id: item.id, sequence: item.sequence, marketplaceStopNumber: item.marketplace_stop_number ?? undefined, address: item.address_full, latitude: item.latitude, longitude: item.longitude, status: item.status, packages: packageByStop.get(item.id) ?? [] }));
...
export async function saveRemoteRoute(route: RouteState, userId: string) {
  ...
  if (route.stops.length) {
    const { error } = await supabase.from("route_stops").upsert(route.stops.map((stop) => ({ id: stop.id, user_id: userId, route_id: route.id, sequence: stop.sequence, marketplace_stop_number: stop.marketplaceStopNumber ?? null, address_full: stop.address, latitude: stop.latitude, longitude: stop.longitude, status: stop.status, completed_at: stop.status === "completed" || stop.status === "failed" ? new Date().toISOString() : null, updated_at: new Date().toISOString() })), { onConflict: "id" });
    if (error) throw error;
  }
  if (route.packages.length) {
    ...
    const { error } = await supabase.from("packages").upsert(route.packages.map((pack) => ({ id: pack.id, user_id: userId, route_id: route.id, stop_id: stopByPackage.get(pack.id) ?? null, tracking_code: pack.trackingCode, barcode_raw: pack.rawBarcode ?? pack.trackingCode, marketplace_stop_number: pack.marketplaceStopNumber ?? null, address_full: pack.address, latitude: pack.latitude, longitude: pack.longitude, status: pack.status, vehicle_position: pack.vehiclePosition ?? null, delivered_at: pack.status === "delivered" ? new Date().toISOString() : null, updated_at: new Date().toISOString() })), { onConflict: "id" });
    if (error) throw error;
  }
